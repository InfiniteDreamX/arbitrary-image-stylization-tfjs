...
/* TO DO
Ya funciona el style transfer con ADAIN, pero hay que hacer el cambio de canales en la imagen
Hay que ver ahora el motivo por el que falla la versión mia. Recomiendo hacerla bien con node y ver que onda.
Finalmente, falta agregar el soporte para el reflect padding, o bien por lo menos arreglar que se vea mas chica.
...
class Main {
    ...
    this.modelSelectTransformer.onchange = (evt) => {
      if (evt.target.value === 'original') {
        ...
        this.loadSeparableTransformerModel()
          ...
          .finally(() => this.enableStylizeButtons());
      } else if (evt.target.value === 'AdaIN') {
        this.disableStylizeButtons();
        this.loadAdain()
          .then((adainModel) => {
            this.transformNet = adainModel;
            this.usingAdain = true;
            console.log(adainModel);
          })
          .finally(() => this.enableStylizeButtons());
      }
    };
  ...
  async startStyling() {
    if (!this.usingAdain) {
      ...
      await tf.nextFrame();
      this.styleButton.textContent =
        'Generating Content Representation';
      await tf.nextFrame();
      let bottleneck = await tf.tidy(()=> {
        console.log(this.transformNet.encoder);
        return this.transformNet.encoder.predict(
          tf.browser
            .fromPixels(this.contentImg)
            .toFloat()
            .div(tf.scalar(255))
      ...
      await tf.nextFrame();
      console.log('Generando la representacion de estilo');
      await tf.nextFrame();
      ...
      await tf.nextFrame();
      bottleneck = await tf.tidy(() => {
        return tf.clipByValue(this.transformNet.decoder.predict(resultAdain).squeeze(), 0, 1);
      });
      await tf.browser.toPixels(bottleneck, this.stylized);
      bottleneck.dispose();
      resultAdain.dispose();
    }
  }
